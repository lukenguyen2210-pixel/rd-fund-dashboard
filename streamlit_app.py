...
def clean_money(value):
    """Xử lý triệt để các ký tự tiền tệ rác để chuyển về số"""
    if pd.isna(value) or str(value).strip() == "":
        return 0
    s = str(value).lower()
    # Xóa 'đ', dấu phẩy, dấu chấm và khoảng trắng
...
def fetch_all_data():
    """Tải toàn bộ dữ liệu các năm một lần để tính Grand Total"""
    all_data = {}
    total_g_income = 0
    total_g_expense = 0
    ...
    for y in YEARS_LIST:
        ...
        all_data[y] = {"income_df": df_inc, "expense_df": df_ex, "y_inc": y_inc, "y_exp": y_exp}
        total_g_income += y_inc
        total_g_expense += y_exp
...
try:
    ...
    # Hiển thị Grand Total Balance (Màu xanh đậm, bo góc)
    st.markdown(f"""
    ...
    # Chọn năm hiển thị chi tiết
    year_choice = st.sidebar.selectbox("Chọn năm xem báo cáo:", YEARS_LIST[::-1])
    current_year_data = data_store[year_choice]
    
    df_members = current_year_data["income_df"]
    df_expense = current_year_data["expense_df"]

    # Metrics năm hiện tại
    ...
    # Bảng chi tiết (Đã fix STT bắt đầu từ 1)
    ...
    with st.expander(f"Nhật ký chi tiêu {year_choice}"):
        if not df_expense.empty:
            ...
            st.info("Chưa có dữ liệu chi tiêu cho năm này.")

except Exception as e:
    st.error(f"Đã xảy ra lỗi khi đồng bộ dữ liệu: {e}")
